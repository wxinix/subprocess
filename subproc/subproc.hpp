...
// =============================================================================
// subproc.hpp — Modern C++23 single-header subproc library for Windows
//
...
enum class PipeOption : int { inherit, cout, cerr, specific, pipe, close, none };
...
struct SubprocError : std::runtime_error { using std::runtime_error::runtime_error; };
struct OSError : SubprocError { using SubprocError::SubprocError; };
struct CommandNotFoundError : SubprocError { using SubprocError::SubprocError; };
struct SpawnError : OSError { using OSError::OSError; };

struct TimeoutExpired : SubprocError {
    using SubprocError::SubprocError;
    TimeoutExpired(const std::string& msg, CommandLine a_cmd, const double a_timeout,
                   std::string a_cout, std::string a_cerr)
        : SubprocError{msg}, cmd{std::move(a_cmd)}, timeout{a_timeout},
          cout{std::move(a_cout)}, cerr{std::move(a_cerr)} {}
    CommandLine cmd; double timeout; std::string cout; std::string cerr;
};

struct CalledProcessError : SubprocError {
    using SubprocError::SubprocError;
    CalledProcessError(const std::string& msg, CommandLine a_cmd, const int64_t retcode,
                       std::string a_cout, std::string a_cerr)
        : SubprocError{msg}, returncode{retcode}, cmd{std::move(a_cmd)},
          cout{std::move(a_cout)}, cerr{std::move(a_cerr)} {}
    int64_t returncode; CommandLine cmd; std::string cout; std::string cerr;
};
...
inline std::u16string utf8_to_utf16(const std::string& input) {
    ...
    const int n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, input.c_str(),
        static_cast<int>(size), buffer.get(), static_cast<int>(size));
...
inline std::string utf16_to_utf8(const std::u16string& input) {
    ...
    const int r = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, wide,
        static_cast<int>(size), nullptr, 0, nullptr, nullptr);
    if (r <= 0) return {};
    const auto buffer = std::make_unique<char[]>(static_cast<size_t>(r));
    const int m = WideCharToMultiByte(CP_UTF8, 0, wide, static_cast<int>(size),
        buffer.get(), r, nullptr, nullptr);
...
struct PipePair {
    ...
    PipePair& operator=(PipePair&& other) noexcept {
        close(); input = other.input; output = other.output; other.disown(); return *this;
    }

    PipeHandle input = kBadPipeValue;
    PipeHandle output = kBadPipeValue;

    void disown() { input = kBadPipeValue; output = kBadPipeValue; }

    void close() { close_input(); close_output(); }
...
private:
    void close_handle(PipeHandle PipePair::*member) {
        if (this->*member != kBadPipeValue) { CloseHandle(this->*member); this->*member = kBadPipeValue; }
    }
};
...
inline void close_and_reset(PipeHandle& h) {
    if (h != kBadPipeValue) { (void)pipe_close(h); h = kBadPipeValue; }
}
...
inline bool pipe_set_blocking(const PipeHandle handle, const bool should_block) {
    ...
    if (should_block) state &= ~PIPE_NOWAIT; else state |= PIPE_NOWAIT;
...
[[nodiscard]] inline int pipe_wait_for_read(const PipeHandle handle, const double seconds) {
    struct BlockingGuard {
        const PipeHandle h; DWORD original_state = 0; bool saved = false;
        ...
        ~BlockingGuard() { if (saved) (void)SetNamedPipeHandleState(h, &original_state, nullptr, nullptr); }
    };
...
[[nodiscard]] inline PipeHandle pipe_file(const std::string_view filename, const std::string_view mode) {
    ...
    if (mode.contains('r')) { access |= GENERIC_READ; disposition = OPEN_EXISTING; }
    if (mode.contains('w')) { access |= GENERIC_WRITE; disposition = CREATE_ALWAYS; }
    if (mode.contains('+')) access |= GENERIC_READ | GENERIC_WRITE;
    const std::u16string wide = utf8_to_utf16(std::string{filename});
    HANDLE hFile = CreateFileW(reinterpret_cast<LPCWSTR>(wide.c_str()), access, 0, nullptr,
...
struct AutoClosePipe {
    ...
    ~AutoClosePipe() { if (m_handle != kBadPipeValue) (void)pipe_close(m_handle); }
...
inline bool setup_redirect_stream(PipeVar& input, const PipeHandle output) {
    return std::visit(overloaded{
        ...
        [output](auto&& source) { pipe_thread(source, output, true); return true; },
    }, input);
}
...
inline std::string join_path(std::string parent, std::string child) {
    ...
    while (child.starts_with("./")) child.erase(0, 2);
...
// detail continued after getenv/abspath are defined (try_exe and find_program_in_path need them).
...
[[nodiscard]] inline std::string get_cwd() { return std::filesystem::current_path().string(); }
inline void set_cwd(const std::string& path) { std::filesystem::current_path(path); }

[[nodiscard]] inline std::string abspath(std::string dir, std::string relative = "") {
    dir = detail::clean_path(dir);
    if (detail::is_absolute_path(dir)) return dir;
    if (relative.empty()) relative = get_cwd();
    if (!detail::is_absolute_path(relative)) relative = detail::join_path(get_cwd(), relative);
    return detail::join_path(relative, dir);
}
...
[[nodiscard]] inline std::string escape_shell_arg(const std::string& arg, const bool escape = true) {
    ...
    std::string result; result.reserve(arg.size() + 4); result += '\"';
...
namespace detail {  // continued — try_exe/find_program_in_path need getenv/abspath above
...
inline void find_program_clear_cache(); // defined after Environ; used by EnvironSetter::operator=
...
public:
    ...
    EnvironSetter& operator=(const T value) {
        if constexpr (std::is_same_v<T, bool>) return *this = std::string{value ? "1" : "0"};
        else return *this = std::to_string(value);
    }
...
[[nodiscard]] inline std::u16string create_env_block(const EnvMap& map) {
    ...
    size_t size = 1;
    for (const auto& [name, value] : map) size += name.size() + value.size() + 2;
...
inline std::string LastErrorString() {
    ...
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
        nullptr, dw, 0, reinterpret_cast<LPTSTR>(&lpMsgBuf), 0, nullptr);
...
struct Popen {
    ...
    [[nodiscard]] bool poll();
    int64_t wait(const double timeout = -1.0);
    [[nodiscard]] std::optional<int64_t> try_wait(const double timeout);
    [[nodiscard]] bool send_signal(SigNum signum) const;
    [[nodiscard]] bool terminate() const { return send_signal(SigNum::PSIGTERM); }
    [[nodiscard]] bool kill() const { return m_soft_kill ? send_signal(SigNum::PSIGTERM) : send_signal(SigNum::PSIGKILL); }
    void close();
...
// =============================================================================
// RunBuilder — fluent API using C++23 deducing this
// =============================================================================
...
// =============================================================================
// find_program (needs Popen for is_python3 detection)
// =============================================================================

namespace detail {

inline bool is_python3(const std::string& path) {
    Popen p(CommandLine{path, "--version"}, {.cout = PipeOption::pipe, .cerr = PipeOption::cout});
    const auto output = pipe_read_all(p.cout);
    (void)p.wait();
    return output.contains("3.");
}
...
[[nodiscard]] inline std::string find_program(const std::string& name) {
    ...
    return (!result.empty() && detail::is_python3(result)) ? result : std::string{};
}

// =============================================================================
// Popen / ProcessBuilder / run() — out-of-class method implementations
// =============================================================================

inline Popen::Popen(const CommandLine& command, RunOptions options) { init(command, options); }
...
inline Popen::Popen(Popen&& other) noexcept { *this = std::move(other); }

inline Popen& Popen::operator=(Popen&& other) noexcept {
    close();
    cin = other.cin; cout = other.cout; cerr = other.cerr;
    pid = other.pid; returncode = other.returncode;
    args = std::move(other.args); m_soft_kill = other.m_soft_kill;
    process_info = other.process_info; other.process_info = {};
    other.cin = kBadPipeValue; other.cout = kBadPipeValue; other.cerr = kBadPipeValue;
    other.pid = 0; other.returncode = kBadReturnCode;
    return *this;
}

inline Popen::~Popen() { close(); }

inline void Popen::close() {
    close_and_reset(cin); close_and_reset(cout); close_and_reset(cerr);
    if (pid > 0) {
        (void)wait();
        (void)CloseHandle(process_info.hProcess);
        (void)CloseHandle(process_info.hThread);
    }
    pid = 0; returncode = kBadReturnCode; args.clear();
}

inline void Popen::handle_wait_result(const DWORD wr) {
    switch (wr) {
    case WAIT_ABANDONED: throw OSError(std::format("WAIT_ABANDONED error:{}", GetLastError()));
    case WAIT_FAILED: throw OSError(std::format("WAIT_FAILED error:{}:{}", GetLastError(), LastErrorString()));
    case WAIT_OBJECT_0: {
        DWORD excode;
    ...
    default: throw OSError(std::format("WaitForSingleObject failed: {}", wr));
    }
}
...
inline bool Popen::send_signal(const SigNum signum) const {
    ...
    case PSIGKILL:
        ...
        detail::for_each_child_process(process_info.dwProcessId,
            [](const PROCESSENTRY32& pe) { detail::TerminateProcessByID(pe.th32ProcessID); });
        break;
    case PSIGINT: result = GenerateConsoleCtrlEvent(CTRL_C_EVENT, 0); break;
    default: result = GenerateConsoleCtrlEvent(CTRL_BREAK_EVENT, pid); break;
    }
    if (!result) std::cerr << "error: " << LastErrorString() << "\n";
    return result;
}
...
inline Popen ProcessBuilder::run_command(const CommandLine& cmdline) const {
    ...
    const auto configure_pipe = [](const PipeOption option, const PipeHandle specific_pipe, PipePair& pair,
        ...
        case specific: pipe_set_inheritable(specific_pipe, true); std_handle = specific_pipe; break;
        case none: std_handle = nullptr; break;
        default: break;
        }
    };
    ...
    if (!this->env.empty()) { envblock = create_env_block(this->env); l_env = envblock.data(); }

    const DWORD process_flags = CREATE_UNICODE_ENVIRONMENT
        | (this->new_process_group ? CREATE_NEW_PROCESS_GROUP : 0)
        | (this->create_no_window ? CREATE_NO_WINDOW : 0)
        | (this->detached_process ? DETACHED_PROCESS : 0);

    const BOOL bSuccess = CreateProcess(program.c_str(), args.data(), nullptr, nullptr, TRUE,
        process_flags, l_env, l_cwd, &siStartInfo, &piProcInfo);
...
[[nodiscard]] inline CompletedProcess run(Popen& popen, const bool check) {
    ...
    if (check)
        throw CalledProcessError{"failed to execute " + popen.args[0], popen.args,
            completed.returncode, completed.cout, completed.cerr};
    return completed;
}
...
[[nodiscard]] inline CompletedProcess run(CommandLine command, const RunOptions& options) {
    ...
    if (!popen.try_wait(options.timeout)) {
        ...
        (void)popen.wait();
        throw TimeoutExpired{"subproc::run timeout reached", command, options.timeout,
            completed.cout, completed.cerr};
    }
    completed.returncode = popen.returncode;
    completed.args = std::move(command);
    if (options.raise_on_nonzero && completed.returncode != 0)
        throw CalledProcessError{"failed to execute " + completed.args[0], completed.args,
            completed.returncode, completed.cout, completed.cerr};
    return completed;
}
